...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  try {
    if (body.id) {
      ...
      // Si viene de una solicitud, márcala como entregada/adoptada.
      if (body.solicitud_id) {
        await env.DB.prepare(`UPDATE solicitudes SET estado = 'aprobada', etapa = 'adoptado' WHERE id = ?`).bind(Number(body.solicitud_id)).run();
      }
