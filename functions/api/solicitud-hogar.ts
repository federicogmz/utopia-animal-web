...
const FIELD_KEYS = [
  ...
  'instagram', 'facebook', 'como_nos_conociste',
  'familiar_nombre', 'familiar_telefono', 'referencia_nombre', 'referencia_telefono',
  // Específicos de hogar temporal
  'por_que_hogar_temporal', 'cuantos_puede_acoger', 'tiempo_disponible', 'experiencia_acogida',
  'espacio_aislamiento', 'disponibilidad_transporte_vet', 'acepta_visitas_seguimiento',
  'cuidador_principal', 'responsable_gastos',
  'plan_dificultad_economica', 'plan_comportamiento', 'plan_enfermedad', 'plan_vacaciones',
  'tiene_otras_mascotas', 'otras_mascotas_tipo', 'otras_esterilizadas', 'otras_alimento', 'otras_anteriores',
...
const INT_FIELDS = new Set([
  'edad', 'estrato', 'mallas_seguridad', 'cuantos_puede_acoger', 'espacio_aislamiento',
  'disponibilidad_transporte_vet', 'acepta_visitas_seguimiento',
  'tiene_otras_mascotas', 'otras_esterilizadas',
...
function toIntFlag(v: unknown): number | null {
  if (v === true || v === 'true' || v === 1 || v === '1' || v === 'si' || v === 'sí') return 1;
  if (v === false || v === 'false' || v === 0 || v === '0' || v === 'no') return 0;
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  for (const field of REQUIRED_FIELDS) {
    const v = data[field];
    if (v === undefined || v === null || v === '' || v === false || v === 0 || v === '0') {
  ...
  const sql = `INSERT INTO solicitudes_hogar (${columns.join(', ')}) VALUES (${placeholders})`;
  const bindings = [ipHash, ...FIELD_KEYS.map(k => values[k]), JSON.stringify(data)];
